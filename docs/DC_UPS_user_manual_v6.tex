...
DC-UPS-2CH автоматически поддерживает питание двух независимых выходов \textbf{ROUTER} и \textbf{ONT}. В обычном режиме устройство питается от сети и поддерживает аккумулятор в заряженном состоянии. При отключении 230~В переход на аккумулятор происходит автоматически, без действий пользователя.
...
\begin{enumerate}
  \item Убедиться, что подключены нужные выходы ROUTER и ONT.
  \item Если на корпусе есть отдельные механические выключатели выходов, перевести требуемые каналы во включённое положение.
  \item Подать 230~В.
...
После аварийного deep sleep устройство просыпается от возврата сети автоматически. В \textbf{режиме хранения} возврат 230~В устройство не будит.
\end{warnbox}
...
\begin{enumerate}
  ...
  \item ROUTER и ONT будут отключены, Wi-Fi выключится, ESP32 уйдёт в deep sleep без периодического таймера.
\end{enumerate}

\textbf{Чтобы снова включить устройство:} один раз нажать сервисную кнопку.

\begin{warnbox}[title=Важно]
В Shelf Sleep устройство просыпается \textbf{только от физической кнопки}. Подключение или возврат 230~В само по себе его не включает.
\end{warnbox}
...
\vspace{7mm}
\begin{center}
\qrcode[height=32mm]{http://dc-ups.local/}\\[2mm]
{\small Локальная веб-панель (при подключении к той же сети и поддержке mDNS).}
\end{center}

\vfill
\begin{tabularx}{\textwidth}{@{}lX@{}}
Контакт изготовителя: & \rule{50mm}{0.4pt} \\
Дата документа: & 02.09.2026 \\
Редакция: & Руководство v6 / Rev. 1 \\
\end{tabularx}
